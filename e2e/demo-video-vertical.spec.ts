...
test('record marketing walkthrough vertical', async ({ page }) => {
  await page.goto('/')
  ...
  if (page.url().includes('/inscription')) {
    ...
    await page.locator('#name').fill('Alex Vertical')
    await page.locator('#email').fill('alex.vertical@example.com')
